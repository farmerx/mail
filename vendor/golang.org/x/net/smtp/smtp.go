...
// DialSSL returns a new Client connected to an SMTP server at addr.
// The addr must include a port, as in "mail.example.com:smtp".
func DialSSL(addr string) (*Client, error) {
	conn, err := net.Dial("tcp", addr)
	if err != nil {
		return nil, err
	}
	host, _, _ := net.SplitHostPort(addr)
	conn = tlsClient(conn, tlsConfig(host))
...
func SendMailUsingSSL(addr string, a Auth, from string, to []string, msg []byte) error {
	c, err := DialSSL(addr)
	if err != nil {
		return err
	}
